...
export default function EventDetailScreen({ route, navigation }) {
  ...
  const WeatherWidget = () => (
    <View style={styles.weatherWidget}>
      <Sun size={18} color={COLORS.primary} style={{ marginRight: 6 }} />
      <Text style={styles.weatherText}>24°C</Text>
      <Text style={[styles.weatherText, { color: COLORS.textSecondary, marginLeft: 4 }]}>Soleado</Text>
    </View>
  );

  const AttendeesPreview = () => {
    // Mock attendees based on MOCK_USERS logic (simulated)
    const attendees = [
    ...
    return (
      <View style={styles.attendeesContainer}>
        <View style={styles.avatarStack}>
          {attendees.slice(0, 4).map((user, index) => (
            <Image 
              key={user.id} 
              source={{ uri: user.avatar }} 
              style={[styles.attendeeAvatar, { marginLeft: index === 0 ? 0 : -12, zIndex: 4 - index }]} 
            />
          ))}
          <View style={[styles.attendeeAvatar, styles.moreAttendeesBadge, { marginLeft: -12, zIndex: 0 }]}>
            <Text style={styles.moreAttendeesText}>+12</Text>
          </View>
        </View>
        <Text style={styles.attendeesLabel}>Ya se han unido</Text>
      </View>
    );
  };
  ...
  const SponsorsCarousel = () => {
    const sponsors = [
      { id: 1, name: 'TechCorp', logo: 'https://logo.clearbit.com/google.com' }, // Placeholder logos
      { id: 2, name: 'BevCo', logo: 'https://logo.clearbit.com/coca-cola.com' },
      { id: 3, name: 'SoundSys', logo: 'https://logo.clearbit.com/spotify.com' },
      { id: 4, name: 'EventPro', logo: 'https://logo.clearbit.com/eventbrite.com' },
    ];
      ...
      <View style={styles.sponsorsContainer}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 4 }}>
          {sponsors.map(sponsor => (
            <View key={sponsor.id} style={styles.sponsorItem}>
          ...
          <View style={styles.mapContainer}>
            ...
            <View style={styles.mapActionsRow}>
              <TouchableOpacity style={styles.mapActionBtn} onPress={handleOpenMaps}>
                ...
                <Text style={styles.mapActionText}>Cómo llegar</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.mapActionBtn} onPress={() => { }}>
                <Car size={16} color={COLORS.primary} />
                <Text style={styles.mapActionText}>Ir en coche</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
                    ...
                    {publicPhotos.map((photo, index) => {
                      ...
                      return (
                        <TouchableOpacity
                          ...
                          style={[styles.galleryItem, { width: '100%', marginBottom: 12, marginRight: 0, height: 120 + (index % 3) * 50 }]}
                        >
                    ...
                    {publicPhotos.map((photo, index) => {
                      ...
                      return (
                        <TouchableOpacity
                          key={photo.id}
                          onPress={() => openLightbox(index)}
                          style={[styles.galleryItem, { width: '100%', marginBottom: 12, marginRight: 0, height: 160 + (index % 2) * 60 }]}
                        >
                          <Image source={{ uri: photo.url }} style={[styles.galleryImage, { width: '100%', height: '100%' }]} />
                        </TouchableOpacity>
                      );
            ...
            >
              ...
              {menuViewMode === 'horizontal' ? (
                ...
                <View style={{ maxHeight: height * 0.7 }}>
                  <ScrollView nestedScrollEnabled={true} showsVerticalScrollIndicator={true} contentContainerStyle={{ paddingBottom: 20 }}>
          ...
          {isOrganizer && (
            ...
            >
              ...
              <View style={styles.infoCard}>
                ...
                {/* <TouchableOpacity
                  style={{ position: 'absolute', top: 20, right: 20, padding: 8, backgroundColor: COLORS.primary + '15', borderRadius: 12 }}
                     ...
                     {event.endDate ? (
                       <View style={{ flex: 1, marginLeft: 12 }}>
                         <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 2 }}>
                           <Play size={10} color={COLORS.primary} style={{ marginRight: 6 }} />
                           <Text style={styles.infoText}>{event.date}</Text>
                         </View>
                         <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                           <Flag size={10} color={COLORS.textSecondary} style={{ marginRight: 6 }} />
                           <Text style={[styles.infoText, { color: COLORS.textSecondary }]}>{event.endDate}</Text>
                         </View>
                       </View>
                     ) : (
                       <Text style={styles.infoText}>{event.date}</Text>
